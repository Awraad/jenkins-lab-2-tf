...
module "tags_bastion" {
  ...
  namespace   = var.name
  environment = "dev"
  name        = "awraad-basion-devops-bootcamp"
  delimiter   = "_"
...
module "tags_webserver" {
  ...
  namespace   = var.name
  environment = "dev"
  name        = "awraad-webserver-devops-bootcamp"
  delimiter   = "_"
...
resource "aws_route53_zone" "awraad_dobc" {
  name = "awraad.dobc"
  tags = module.tags_network.tags
...
resource "aws_route53_record" "webserver" {
  zone_id = aws_route53_zone.awraad_dobc.id
  name    = "webserver"
  type    = "A"
  ttl     = 300
...
resource "aws_instance" "webserver" {
  ...
  depends_on                  = [aws_instance.api]
    provisioner "local-exec"{
       command = "echo ${aws_instance.api.0.public_ip} > public_ip_api.txt"
      }
}
